...
console.log(a !== b);
//3.논리
console.log(false) //and 연산자
;
console.log(true) //or 연산자
;
console.log(0) //and 연산자는 왼쪽에서 오른쪽으로 진행하면서 거짓을 만나면(0) 바로 반환
;
console.log(3) // 거짓을 만나지 않는다면 맨 마지막 참을 출력 한다(3)
;
console.log(NaN) //모두 거짓을 만나면 맨 처음 거짓을 출력(NaN)
;
console.log(1) // or 연산자는 왼쪽에서 오른쪽으로 가면서 제일 처음 만나는 참 데이터를 출력(1)
;
console.log(NaN) // 모두 거짓이라면 맨 마지막 거짓 내역을 출력(NaN)
;
// 4.Nullish 병합
const num1 = 0;
...
function number(num) {
    ...
    else if (num < 0) console.log("음수입니다");
    else console.log("0입니다");
}
number(1);
number(-1);
number(0);
...
for(let a1 = 0; a1 < 10; a1++){
    if (a1 % 2 == 0) continue; // a가 2의 배수일 경우 다시 조건문으로 올라가 for문을 계속 진행
    console.log(a1) // 홀수만 출력된다
    ;
}
